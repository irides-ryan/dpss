...
MainWindow::MainWindow(QWidget *parent) :
  ...
  QObject::connect(m_controller, &SController::accept,
                   [=](QSX::TcpHandler *handler) {
                     QObject::connect(handler, &QSX::TcpHandler::connecting,
                                      [=](QSS::Address &address) {
                                        qDebug() << handler << "connecting"
                                                 << QString(address.toString().c_str());
                                      });

                     QObject::connect(handler, &QSX::TcpHandler::connected,
                                      [=](int latency) {
                                        qDebug() << handler << "connected:"
                                                 << latency << "ms";
                                      });

                     QObject::connect(handler, &QSX::TcpHandler::bytesRead,
                                      [=](uint64_t s) {
                                        in += s;
                                      });

                     QObject::connect(handler, &QSX::TcpHandler::bytesWrite,
                                      [=](uint64_t s) {
                                        out += s;
                                      });
